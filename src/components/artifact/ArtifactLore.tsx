  ...
  return (
    <div className={`space-y-12 ${className}`}>
      ...
      <section className="animate-in fade-in slide-in-from-right-4 duration-500">
        <h3 className="text-[9px] uppercase tracking-[0.5em] text-gold-600 mb-6 font-mono border-b border-gold-500/10 pb-2 inline-block">
          Primary Analysis
        </h3>
        ...
        <div className="space-y-8">
          {artifact.lore && artifact.lore.length > 0 ? (
            artifact.lore.map((line, idx) => (
              <p 
                key={idx} 
                className="text-stone-400 font-serif text-lg leading-loose border-l border-gold-500/20 pl-8 py-2 hover:border-gold-500/50 transition-all duration-300 hover:text-stone-200"
              >
                {line}
              </p>
            ))
          ) : (
            <p className="text-stone-600 font-serif italic pl-8 border-l border-gold-500/10 py-2">
