...
export const TerminalGrid: React.FC<TerminalGridProps> = ({ 
  ...
  return (
    <div className={cn(
      "terminal-container crt-container min-h-[600px] w-full p-8 font-mono text-[#d4af37] selection:bg-[#d4af37] selection:text-[#020617] overflow-hidden",
      isGlitching && "terminal-glitch"
    )}>
      {/* High-Fidelity CRT Overlays */}
      <div className="crt-overlay" />
      <div className="scanline" />
      
      <div className="relative z-10 grid grid-cols-12 gap-8 border border-[#d4af37]/20 bg-black/40 p-6 backdrop-blur-xl shadow-[0_0_50px_rgba(212,175,55,0.1)] rounded-sm h-[700px] overflow-hidden">
        
        {/* Header Area */}
        <div className="col-span-12 mb-4 border-b border-[#d4af37]/20 pb-4 phosphor-glow">
              ...
              {Array.from({ length: 4 }).map((_, i) => (
                <div 
                  ...
                  className={cn(
                    ...
                    i < attemptsRemaining ? "bg-[#d4af37] shadow-[0_0_15px_#d4af37]" : "bg-transparent opacity-20"
                  )}
                />
              ))}
            </div>
        ...
        {/* Matrix Area */}
        <div className="col-span-8 grid grid-cols-2 gap-x-12 overflow-hidden h-[500px] leading-tight text-sm sm:text-base select-none phosphor-glow">
          {[0, 1].map((side) => (
                    ...
                    {line.content.map((part, j) => (
                      typeof part === 'string' ? (
                        ...
                        <span 
                          ...
                          className="font-bold cursor-pointer hover:bg-[#d4af37] hover:text-[#020617] transition-colors duration-75 px-1 interactive-bloom"
                        >
                          {part.word}
                        </span>
                      )
        ...
        <div className="col-span-4 border-l border-[#d4af37]/20 pl-6 flex flex-col h-[500px] phosphor-glow">
          <div className="flex-1 overflow-y-auto space-y-3 text-xs font-bold scrollbar-hide">
            {logs.map((line, i) => (
              <p key={i} className={cn(
                "animate-in fade-in slide-in-from-left-2 duration-300",
                line.includes('GRANTED') ? "text-green-400" : line.includes('DENIED') ? "text-red-400" : ""
              )}>
                {line}
              </p>
            ))}
